...
import org.bcommon.resources.Resources;
 ...
 * {@link org.bwizard.models.StaticModel} and add a number of {@link WizardStep}
 * instances.
 * 
 * <pre>
 ...
 * @see org.bwizard.models.StaticModel
 * @see org.bwizard.models.DynamicModel
 * @see org.bwizard.models.MultiPathModel
 */
public class Wizard extends JPanel {
	...
	/**
	 * When specified as the {@link #setDefaultExitMode(int) defaultExitMode},
	 * this causes the wizard to continue displaying the final step after
	 * finished has been pressed. The wizard is closed when the user presses the
	 * close button. This allows the wizard to display a final confirmation
	 * screen.
	 * 
	 * @see #EXIT_ON_FINISH
	 */
	...
	/**
	 * When specified as the {@link #setDefaultExitMode(int) defaultExitMode},
	 * this causes the wizard to exit immediately once finish is pressed and
	...
	public static Resources wresources = new Resources("bwizard");
	...
	public Wizard(WizardModel model) {
		...
		p.setBorder(BorderFactory.createEmptyBorder(BORDER_WIDTH, BORDER_WIDTH,
		...
		separator.setBorder(BorderFactory.createLineBorder(Color.lightGray));
		commandPanel.add(separator, BorderLayout.NORTH);
		...
		if (model instanceof OverviewProvider) {
			...
			panel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
					.createEmptyBorder(0, 0, 0, 2), BorderFactory
					.createEtchedBorder()));
			panel.add(((OverviewProvider) model).getOverviewComponent(),
					BorderLayout.CENTER);
			overviewContainer.add(panel, BorderLayout.WEST);
			overviewContainer.add(Box.createVerticalStrut(buttonBar
	...
	public void setDefaultExitMode(int defaultExitMode) {
		if (defaultExitMode != EXIT_ON_FINISH
				&& defaultExitMode != EXIT_ON_CLOSE)
	...
	/**
	 * Called by the constructor to create the wizards title component. The
	 * default component will be an instance of {@link DefaultTitleComponent}
	 * but subclasses may override to provide a custom implementation. Typically
	 * the title component listens to the wizard model and updates its
	...
	/**
	 * Gets the component being used to render the wizards title. By default
	 * this will be an instance of {@link DefaultTitleComponent}. Subclasses
	 * can change the default by overriding {@link #createTitleComponent()}.
	 * <p>
	 * For example, to activate the gradient background on the default title
	 * component you can call.
	 * 
	 * <pre>
	 * ((DefaultTitleComponent) wizard.getTitleComponent())
	...
	/**
	 * Called by the constructor to create the button bar. Subclasses may
	 * override to provide a custom {@link ButtonBar} implementation.
	 * 
	...
	/**
	 * Checks the visibily of the overview panel that is displayed on the
	 * wizards left panel. The overview panel will only be displayed if this
	 * property is <tt>true</tt> and the current {@link WizardModel}
	 * implements {@link OverviewProvider}.
	 */
	 ...
	 *            <tt>true</tt> to display the overview, <tt>false</tt>
	 *            otherwise.
	 */
	...
	private void configureOverviewContainer() {
		overviewContainer.setVisible(overviewVisible
				&& model instanceof OverviewProvider);
	}
	...
	/**
	 * Marks this wizard as finished. This will cause the button bar to only
	 ...
	 * {@link #activeStep} is {@link WizardStep#isBusy busy}. This method
	 * displays a {@link JOptionPane} asking if the user wants to abort the
	 * wizard.
	 * 
	...
	protected boolean confirmAbort() {
		int response = JOptionPane.showConfirmDialog(this,
				"Cancel the currently active task?", "Abort",
	...
	private void handleStepChange() {
		...
		// setTitle("");// TODO: esto he aumentado
		activeStep.prepare();
		handleViewChange();
	}

	/**
	 * TODO: esto he aumnetado
	 */
	// @Override
	// public void setTitle(String arg0) {
	// String title = "";
	// if (activeStep != null)
	// title += "Paso: " + activeStep.getName();
	// else
	// title += "";
	// if (getName() != null)
	// title += " - " + getName();
	...
	private Dimension calculatePreferredStepSize() {
		...
		for (Iterator<WizardStep> iter = getModel().stepIterator(); iter
				.hasNext();) {
			Dimension d = iter.next().getPreferredSize();
	 ...
	 * frame will be automatically closed when the wizard is completed or
	 * canceled. This method will not block.
	 * 
	 * @param title
	 ...
	 *            the title of the frame.
	 * @param relativeTo
	 *            the new {@link JFrame} will be displayed relative to this
	...
	/**
	 * Displays the wizard in a new {@link JDialog} with the specified title.
	 * The dialog will be automatically closed when the wizard is completed or
	 * canceled. This method will block if the dialog is modal.
	 * 
	 * @param title
	 *            the dialog title.
	 * @param parent
	 ...
	 * @param modal
	 *            <tt>true</tt> to make the dialog modal,
	...
	public void showInDialog(String title, Component parent, boolean modal) {
		...
		if (parent != null) {
			...
			if (w instanceof Frame) {
				...
				throw new IllegalArgumentException(
						"Parent component must be within a Frame or Dialog");
			}

			if (parent instanceof Window) {
				dialog = createDialogFor((Window) parent, title, modal);
			} else {
				dialog = createDialogFor(SwingUtilities
						.getWindowAncestor(parent), title, modal);
			}
		} else {
			dialog = new JDialog();
	 ...
	 * @param relativeTo
	 *            the component used to position the window. If the component is
	 *            <tt>null</tt>, the window will be centered on the desktop
	 *            as per
	 *            {@link JWindow#setLocationRelativeTo(java.awt.Component)}.
	 */
	private void showInWindow(Window window, Component relativeTo) {
		((RootPaneContainer) window).getContentPane().add(this);
		// this.getContentContainer());// TODO:solo le coloque el contenido
	 ...
	 *            the title of the frame.
	 * @param desktopPane
	 *            the desktop in which to display the wizard.
	 * @see #addWizardListener
	 * @see #removeWizardListener
	 */
	public void showInInternalFrame(String title, Icon frameIcon,
			JDesktopPane desktopPane) {
		showInInternalFrame(title, frameIcon, true, true, true, true,
				desktopPane);
	}
	 ...
	 * @param maximizable
	 *            if <code>true</code>, the internal frame can be maximised
	 * @param iconifiable
	 ...
	 * @see #removeWizardListener
	 */
	public JInternalFrame showInInternalFrame(String title, Icon frameIcon,
			boolean resizable, boolean closable, boolean maximizable,
			boolean iconifiable, JDesktopPane desktopPane) {
		...
		WizardInternalFrameCloser.bind(this, frame);
		frame.pack();
		...
		try {
			...
			Dimension screenSize = desktopPane.getParent().getSize();

			int newX = (screenSize.width - size.width) / 2;
			int newY = (screenSize.height - 32 - size.height) / 2;
	...
	protected class OverviewBorder extends AbstractBorder {
