...
public class DefaultTitleComponent extends JPanel {
	...
	protected DefaultTitleComponent(Wizard wizard) {
		...
		summary.setFont(new Font("Dialog", Font.PLAIN, 11));
		summary.setEnabled(false);
		summary.setForeground(new Color(51, 51, 51));
		summary.setDisabledTextColor(Color.BLACK);
		summary.setBorder(BorderFactory.createEmptyBorder(
				Wizard.BORDER_WIDTH / 2, Wizard.BORDER_WIDTH, 0,
				Wizard.BORDER_WIDTH));
	...
	/**
	 * Checks if the title is using a gradient background. Subclasses can
	 * control the appearance of the gradient by overriding
	 * {@link #prepareGradient()}.
	 * 
	 * @return <tt>true</tt> if the background will use a gradient,
	 *         <tt>false</tt> if the background will be painted using the
	 *         background color.
	 * @see #prepareGradient()
	 */
	...
	/**
	 * Configures the title to use a gradient background. Subclasses can control
	 ...
	 * 
	 * <pre>
	 * new GradientPaint(getWidth()/2, 0, Color.WHITE, getWidth(), 
	 *    getHeight(), COLOR_GRAY_230));
	 * </pre>
	 * 
	 * If this method returns null, the component will be filled with the
	 * current background color.
	 * 
	 * @return the background paint to use for this component or <tt>null</tt>
	 *         to use the default background color.
	 */
	protected Paint prepareGradient() {
		return new GradientPaint(getWidth() / 2, 0, getBackground(),
				getWidth(), getHeight(), getFadeColor());
	}
	 ...
	 * gray(230,230,230).
	 * 
	 * @return the second color of the gradient. The first color is the
	 ...
	 * 
	 * @param fadeColor
	 *            the second color of the gradient. The first color is the
	 *            background of the component.
	 */
