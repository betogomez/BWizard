...
public class DynamicModel extends AbstractWizardModel {
	...
	private DynamicModelOverview overview;
	 ...
	 * {@link #add(WizardStep, Condition) add(step, (Condition)step)}, other
	 * wise it is equivalent to calling
	 * {@link #add(org.bwizard.WizardStep, Condition) add(step,
	 * TRUE_CONDITION)}.
	 * <p>
	 * This allows the easy use of {@link WizardStep}s that determine the
	 * condition under which they are displayed.
	 * 
	 * @param step
	 *            the step to added.
	 */
	 ...
	 * 
	 * @param step
	 *            the {@link org.bwizard.WizardStep} to add.
	 * @param condition
	 *            the {@link Condition} under which it should be included in the
	 *            wizard.
	 */
	 ...
	 * {@link Condition#evaluate(org.bwizard.WizardModel)}.
	 * <p>
	...
	/**
	 * Returns true if all included steps in the wizard return <tt>true</tt>
	 * from {@link WizardStep#isComplete}. This is primarily used to determine
	 * if the last button can be enabled.
	 * <p>
	 * Please note that this method ignores all steps for which their
	 * {@link Condition} returns false.
	 * 
	 * @return <tt>true</tt> if all the visible steps in the wizard are
	...
	private WizardStep findNextVisibleStep(WizardStep currentStep) {
		int startIndex = (currentStep == null) ? 0
	...
	public JComponent getOverviewComponent() {
		...
		if (overview == null) {
			overview = new DynamicModelOverview(this);
		}
