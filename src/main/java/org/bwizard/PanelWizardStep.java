 ...
 * 
 * <pre>
 * public class MyWizardStep extends PanelWizardStep {
 * 
...
public class PanelWizardStep extends JPanel implements WizardStep {
	 ...
	 * <tt>true</tt> can the wizard progress. This is a bound property.
	 */
	...
	/**
	 * Creates a new step with the specified name and summary. The name and
	...
	/**
	 * Sets this steps summary. This will be displayed in the title of the
	 * wizard while this step is active. The summary is typically an overview of
	 * the step or some usage guidelines for the user.
	 * 
	 * @param summary
	 ...
	 * 
	 * @param icon
	 *            the {@link javax.swing.Icon} that represents this step, or
	 *            <tt>null</tt> if the step doesn't have an icon.
	 */
	public void setIcon(Icon icon) {
		if ((this.icon != null && !this.icon.equals(icon)) || this.icon == null
				&& icon != null) {
			Icon old = this.icon;
			this.icon = icon;
	...
	/**
	 * Checks if the current task is busy. This usually indicates that the step
	 * is performing a time consuming task on a background thread.
	 * 
	 * @return <tt>true</tt> if step is busy performing a background
	...
	/**
	 * Sets the busy state of this wizard step. This should usually be set when
	 * a time consuming task is being performed on a background thread. The
	 * Wizard responds by disabling the various buttons appropriately.
	 * <p>
	 * Wizard steps that go into a busy state must also implement
	 * {@link #abortBusy} to cancel any in progress operation.
	 * 
	 * @param busy
	 *            <tt>true</tt> to mark the step as busy and disable further
	 *            user action, <tt>false</tt> to return the wizard to its
	 *            normal state.
	 */
	...
	/**
	 * Called to prepare this step to display. Subclasses should query the model
	 ...
	 * active.
	 * <p>
	 * If this method will take a long time to complete, subclasses should
	 * consider executing the work and a separate thread and displaying some
	 ...
	 * {@link #isBusy busy} state. Steps that are never busy need not override
	 * this method.
	 */
